...
function fallbackTemplates(inp: BrandInputs) {
  const base = (s: string) => s.trim() ? s : "your brand";
  return [
    { title: "Introduction",      post: `Meet ${base(inp.product)} — ${inp.themes}. Why it fits you.` },
    { title: "Benefits",          post: `3 reasons ${base(inp.product)} is a must-have. (${inp.themes})` },
    { title: "Social proof",      post: `Spotlight review: customers on ${base(inp.product)}. Share yours!` },
    { title: "Behind the scenes", post: `How we make it: a peek behind ${base(inp.product)}.` },
    { title: "Offer",             post: `This week’s deal: don’t miss out. (${inp.themes})` },
  ];
}
function fallbackCaption(inp: BrandInputs) {
  ...
  return `Every day is a new chance to grow with ${inp.product}. ${h}`;
}
function fallbackSlogan(inp: BrandInputs) {
  return `${inp.product}: where ${inp.themes || "quality and feeling"} come together.`;
}
function fallbackHashtags(inp: BrandInputs) {
  const base = (inp.themes || "brand marketing growth")
...
export default function BrandVoicePage() {
  ...
  async function generate(kind: "templates" | "caption" | "slogan" | "hashtags") {
    setErr(null);
    if (!baseValid) { setErr("Enter at least the product/brand."); return; }
    if (credits < COST) { setErr(`Insufficient credits (${credits}). Required: ${COST}.`); return; }
    setBusy(kind);
  ...
  function sendToCampaignBuilder() {
    const seed = {
      brand: { product, themes, primaryColor, logoUrl, caption: outCaption || "", slogan: outSlogan || "", hashtags: outHashtags || [], templates: outTemplates || [] },
      ts: Date.now()
    };
    try { localStorage.setItem("campaign.brandSeed", JSON.stringify(seed)); } catch {}
    alert("Brand info prepared for Campaign Builder. Open that page and click 'Import from Brand Voice'.");
  }
  ...
  return (
    <PageShell title="Brand Voice" desc="Create your brand voice and reusable building blocks (4 credits per generation).">
      <Card>
        <SectionTitle title="Brand inputs" desc="Tell us about your brand. This drives all generations." />
        <div className="grid-2">
          <div>
            <label className="label">Product / Brand</label>
            <input className="input" value={product} onChange={(e) => setProduct(e.target.value)} placeholder="e.g., Dog toys brand" />
          </div>
          <div>
            <label className="label">Primary color (optional)</label>
            <input className="input" value={primaryColor} onChange={(e) => setPrimaryColor(e.target.value)} placeholder="#F5C044 or 'gold'" />
          </div>
          <div style={{gridColumn:"1 / -1"}}>
            <label className="label">Themes / feelings</label>
            <textarea className="textarea" value={themes} onChange={(e) => setThemes(e.target.value)} placeholder="e.g., playful, warm, empathetic; focus on pet love, joy, safety" />
            <div className={TOKENS.SUBTLE} style={{fontSize:12, marginTop:6}}>Use commas or spaces; we’ll make it work.</div>
          </div>
          <div>
            <label className="label">Logo upload (optional)</label>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <input type="file" accept="image/*" onChange={(e) => onPickLogo(e.target.files?.[0] || null)} />
              {logoUrl && <a href={logoUrl} target="_blank" rel="noreferrer" className="subtle" style={{textDecoration:"underline", fontSize:12}}>View logo</a>}
            </div>
          </div>
          <div>
        ...
        <div style={{marginTop:14, display:"flex", flexWrap:"wrap", gap:8, alignItems:"center"}}>
          <GoldCTA onClick={() => generate("templates")} disabled={!baseValid || busy !== null}>{busy === "templates" ? "Working…" : "Generate Templates (4 cr.)"}</GoldCTA>
          <GlowButton onClick={() => generate("caption")} disabled={!baseValid || busy !== null}>{busy === "caption" ? "Working…" : "Generate Caption (4 cr.)"}</GlowButton>
          <GlowButton onClick={() => generate("slogan")} disabled={!baseValid || busy !== null}>{busy === "slogan" ? "Working…" : "Generate Slogan (4 cr.)"}</GlowButton>
          <GlowButton onClick={() => generate("hashtags")} disabled={!baseValid || busy !== null}>{busy === "hashtags" ? "Working…" : "Generate Hashtags (4 cr.)"}</GlowButton>
          {hydrated && (<div className={TOKENS.SUBTLE} style={{fontSize:12, marginLeft:8}}>Your credits: <strong style={{color: credits < 4 ? "#fca5a5" : "#86efac"}}>{credits}</strong></div>)}
      ...
      {(outTemplates.length > 0 || outCaption || outSlogan || outHashtags.length > 0) && (
        <Card>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
            <SectionTitle title="Results" desc="Save to your notes or send to the Campaign Builder." />
            <GlowButton onClick={sendToCampaignBuilder}>Use in Campaign Builder</GlowButton>
          </div>

          {outTemplates.length > 0 && (
            <div style={{marginTop:16}}>
              <div style={{fontWeight:600, fontSize:14, marginBottom:8}}>Templates</div>
              <div className="grid-2">
