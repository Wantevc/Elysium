...
/** Fallback client-canvas (emergency render) */
...
/** NEW: client-side compositing on top of AI background */
async function composeOverBackground({
  ...
  // 3) Cover background (crop to fill)
...
export default function OfferVisualPage() {
  ...
  async function doGenerate(mode: "gen" | "variant") {
    ...
    if (!canGenerate) {
      setErr("Fill in both prompt and offer, and ensure you have enough credits.");
      return;
    }

    setBusy(mode);
    try {
      // 1) Call the server route
      ...
      try {
        ...
        out = r ?? null;
        // 🔎 Show server error explicitly (step 2)
      ...
      // 2) Only use the server image if we actually got a URL
      if (out?.ok && out.url) {
        setImageUrl(out.url);
        setSeed(out.seed || out.url);
      } else {
        // Keep fallback ON so you always see something in dev
  ...
  function clearAll(){
    setPrompt(""); setOffer(""); setLogoUrl(""); setImageUrl(""); setSeed(null); setErr(null);
  }
  ...
  return (
    <PageShell title="Offer + Visual" desc="Generate any picture you like.">
      <Card>
        <SectionTitle
          title="Input"
          desc="Tell us what you would like to create. Use concrete details."
        />

        <div className="grid-2">
          {/* Prompt */}
          <div style={{ gridColumn: "1 / -1" }}>
            <label className="label">Prompt (what would you like to see)</label>
            <textarea
              className="textarea"
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder={`e.g. "A forest in winter with a soft vibe, a man holding a sign that says 'Welcome'"`}
            />
          </div>

          {/* Text / feeling */}
          <div>
            <label className="label">Theme / Feeling</label>
            <input
              className="input"
              value={offer}
              onChange={(e) => setOffer(e.target.value)}
              placeholder={`e.g. "Ice cold" / "Sunny days"`}
            />
          </div>
        </div>
        ...
        >
          <GoldCTA
            onClick={() => doGenerate("gen")}
            disabled={!canGenerate || busy !== null}
          >
            {busy === "gen" ? "Working…" : "Generate (5 cr.)"}
          </GoldCTA>

          <GlowButton
            onClick={() => doGenerate("variant")}
            disabled={!imageUrl || credits < COST || busy !== null}
          >
            {busy === "variant" ? "Working…" : "Make variant (5 cr.)"}
          </GlowButton>

          {hydrated && (
            <div className={TOKENS.SUBTLE} style={{ fontSize: 12, marginLeft: 8 }}>
              Your credits:{" "}
      ...
      <Card>
        <SectionTitle
          title="Preview"
          desc={'If you are not happy with the result, click "Again".'}
        />
        <div
          className="card"
          style={{
          ...
          >
            {imageUrl ? (
              <img
                src={imageUrl}
                alt="preview"
                style={{ maxHeight: 560, width: "auto", borderRadius: 12 }}
              />
            ) : (
              <div
                className={TOKENS.SUBTLE}
                style={{
                  height: 320,
                  width: "100%",
                  display: "grid",
        ...
        <div style={{ marginTop: 12, display: "flex", gap: 8, flexWrap: "wrap" }}>
          <GlowButton onClick={downloadImage} disabled={!imageUrl}>
            Download image
          </GlowButton>
          <button
            type="button"
