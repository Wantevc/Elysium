...
type AuthResp = { loggedIn: boolean; hasToken: boolean };
...
export default function Page() {
  ...
  useEffect(() => {
    const saved = typeof window !== "undefined" ? localStorage.getItem("selected_page_id") : null;
    if (saved) setPageId(saved);
    (async () => {
      try {
        const a = await jfetch<AuthResp>("/api/meta/auth");
        setAuth(a);
      } catch {}
    })();
  }, []);
    ...
    <div className="min-h-screen p-6 md:p-10 bg-gray-50">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <section className="p-4 rounded-2xl bg-white shadow space-y-2">
          <div className="text-sm text-gray-700">Auth: {auth ? JSON.stringify(auth) : "…"}</div>
          <div className="text-xs text-gray-500">
            Tip: auth moet <code>{`{ "loggedIn": true, "hasToken": true }`}</code> zijn.
          </div>
        </section>
        ...
        <section className="p-4 rounded-2xl bg-white shadow space-y-3">
          ...
          <p className="text-sm text-gray-600">
            Vereist permissies in je Meta app: <code>instagram_basic</code> + <code>instagram_content_publish</code>, en IG Business gelinkt aan je Page.
          </p>
          <div className="grid gap-2">
            <input
              value={imageUrl}
